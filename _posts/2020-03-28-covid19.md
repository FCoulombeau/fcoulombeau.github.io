...
Le nuage de points est donné par la liste $\left(\left[X_i;Y_i\right]\right)_{1\leqslant i\leqslant n}$ de ses coordonnées. Ici les abscisses $\left(X_i\right)_{1\leqslant i\leqslant n}$ sont les dates des mesures effectuées et $\left(Y_i\right)_{1\leqslant i\leqslant n}$ le nombres de malades fournis par les gouvernement jour après jour.
...
De plus, pour que $L_i=G_c(Y_i)$ soit défini, il faut que $c>\max\left(Y_i\right)_{1\leqslant i\leqslant n}$.

Le principe de l'algorithme est alors le suivant :
- on pose $c_0=2\times\max\left(Y_i\right)_{1\leqslant i\leqslant n}$;
