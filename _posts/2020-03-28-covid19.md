...
Et $\left(Y_i\right)_{1\leq i\leq n}$ sont les nombres de malades fournis par les gouvernement jour après jour.
...
En utilisant le développement limité de $F_c$ au voisinage de $L_i$ on obtient une expression approximative de $S$ : $$S(a,b,c)\approx\displaystyle\sum_{i=1}^n\left[F_c'(L_i)\left(aX_i+b-L_i\right)\right]^2=\sum_i\left(aX_i+b-L_i\right)^2aY_i\left(1-\frac{Y_i}{c}\right)$$
