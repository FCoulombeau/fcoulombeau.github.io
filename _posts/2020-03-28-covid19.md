...
def fp(a,b,c):
    """Renvoie la dérivée de la sigmoïde."""
    return lambda x:a*c*np.exp(-(a*x+b))/(1+np.exp(-(a*x+b)))**2
...
class covid():
    ...
    def repGraph(self, k, typ=[], err=1.5, prec = 1e-5):
        self.calc([k],err=err,prec=prec)
        repGraph(*self.datas[k],self.L[k],typ=typ, err=err, prec=prec)
    
    def compare(self,typ, which=None, err=1.5, prec = 1e-5):
        fig = plt.figure()
        plt_set_fullscreen()
        ...
        if not which:
            which=range(n)
        self.calc(which,err=err,prec=prec)
        n = len(which)
        p = int(np.sqrt(n+1))
