...
- on continue ainsi jusqu'à ce que $c_{j+1}-c_j$ soit, en valeur absolue, inférieur à une précision voulue;
...
class covid():
    ...
    def repGraph(self, k, typ=[], err=1.5):
        repGraph(*self.datas[k],typ=typ)
    
    def compare(self,typ, which=None, err=1.5):
        fig = plt.figure()
        plt_set_fullscreen()
