...
- on continue ainsi jusqu'à ce que $c_{j+1}-c_j$ soit inférieur à une précision voulue;
...
def interpSigm(data, fin, S0=0, mxS=1e6, err=1.5, prec=1e-3,method = 1):
    ...
    if method:
        ...
        m,L,M = interpSigm2(data,fin,
                        Inf=max(np.max(somcum)+2,c/2), Sup=c*2,
...
class covid():
    ...
    def repGraph(self, k, typ=[], err=1.5, method = 1):
        repGraph(*self.datas[k],typ=typ, method = method)
    
    def compare(self,typ, which=None, err=1.5, method = 1):
        fig = plt.figure()
        plt_set_fullscreen()
