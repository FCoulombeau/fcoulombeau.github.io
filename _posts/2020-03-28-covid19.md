...
Le nuage de points est donné par la liste $\left(\left[X_i;Y_i\right]\right)_{1\leq i\leq n}$ de ses coordonnées. Ici les abscisses $\left(X_i\right)_{1\leq i\leq n}$ sont les dates des mesures effectuées et $\left(Y_i\right)_{1\leq i\leq n}$ le nombres de malades fournis par les gouvernement jour après jour.
