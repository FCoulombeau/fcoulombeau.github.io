...
Enfin, je consacre un [dernier paragraphe](#communications-officielles) aux nombres de cas communiqués par les autorités françaises à partir du 8 mars... dont on peut singulièrement douter de la bonne foi.
...
```python
# Nombre de nouveaux cas pas jours - à partir du 25 février 2020
nouveaux = [0,2,4,20,19,43,30,61,21,73,138,190,336,177,65]
n = len(nouveaux)
# Période utilisée pour effectuer la régression linéaire
...
On commence à constater une grosse différence entre simulation et nombre de cas réels communiqués par les autorités.

Il faut rester très prudent sur la validité de cette simulation mais ce ne serait pas la première fois que ce gouvernement ment sans vergogne...

Pour cette raison, voici deux graphiques : le premier est fait en ne tenant compte, pour la simulation, que des cas communiqués jusqu'au 7 mars - date à partir de laquelle le gouvernement a décidé de ne plus tester que les cas graves.

![CoViD-19 sigmoïdale, évolution du nombre de cas](/img/Cov1003s2.png)

Le second ne prend en compte que les cas communiqués jusqu'au 8 mars.

![CoViD-19 sigmoïdale, évolution du nombre de cas](/img/Cov1003s.png)

Même le graphique obtenu en prenant tous les nouveaux cas donnés par le gouvernement jusqu'à aujourd'hui (9 mars au soir) est très bizarre et... dissonant.

Une petite analyse des deux graphiques précédents : dans les deux cas, si le modèle utilisé est bon, le nombre de personnes qui seront atteintes en France à la fin de l'épidémie semble être inférieur à 10 000. La fin de l'épidémie arriverait - toujours dans le cas où ce modèle serait valable - aux environs de la fin du mois de mars.